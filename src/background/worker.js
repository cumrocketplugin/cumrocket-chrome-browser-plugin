...
async function handle(message, sender) {
  ...
  if (message.type === 'entertainers.settings') {
    await store.saveEntertainers(message.settings);
    await broadcast();
    return true;
  }
  ...
  await store.update(s => {
    switch (message.type) {
      ...
      default: throw new Error('Unknown CumRocket action.');
    }
  });
  await broadcast();
...
chrome.runtime.onMessage.addListener((message, sender, respond) => {
  ...
  handle(message, sender).then(data => respond({ ok: true, data }), error => respond({ ok: false, error: error.message || 'CumRocket could not complete this action.' }));
  return true;
});
