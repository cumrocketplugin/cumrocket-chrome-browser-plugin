...
const secrets = { openai: 'cumrocket.apiKey', anthropic: 'cumrocket.anthropicApiKey' };
  ...
  return {
    ...
    recordCounts(page, observations) {
      const next = queue.then(async () => {
        ...
        const storageKey = `cumrocket.counts.v1.${page}`;
...
export function createTabStore(area = chrome.storage.session) {
  const key = id => `cumrocket.scroll.v1.${id}`;
  let queue = Promise.resolve();
  ...
  return {
    ...
    prune(ids) { return serialize(async () => {
      ...
      const stale = Object.keys(await area.get(null)).filter(k => k.startsWith('cumrocket.scroll.v1.') && !keep.has(k));
...
export function createVisitStore(local = chrome.storage.local, session = chrome.storage.session) {
  const settingsKey = 'cumrocket.visits.settings.v1';
  const historyKey = 'cumrocket.visits.history.v1';
  let queue = Promise.resolve();
  const serialize = work => {
    const next = queue.catch(() => {}).then(work); queue = next; return next;
  };
