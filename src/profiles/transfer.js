...
function validatePackage(data, scope) {
  ...
  if (data.format !== 'cumrocket.profiles' || data.version !== 1) throw new Error('Unsupported profile export format or version.');
...
export function exportProfiles(profiles, scope, id) {
  ...
  const data = { format: 'cumrocket.profiles', version: 1, scope, profiles: selected };
