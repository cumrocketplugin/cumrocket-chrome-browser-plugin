...
$('#sidebar-mode').addEventListener('change', async () => {
  ...
  try {
    ...
    report(sidebar ? 'Sidebar mode saved.' : 'Popup mode saved. Use the toolbar icon to open CumRocket.');
...
async function download(scope, id) {
  ...
  const link = element('a', undefined, { href: url, download: scope === 'all' ? 'cumrocket-profiles.json' : 'cumrocket-profile.json' });
