...
const playing = video => video && !video.paused && !video.ended && video.readyState >= 2 && video.currentTime > 0;
// Select the dominant visible video first. A recognized overlay in that same
// player may supply playback while the underlying main video remains paused.
export function mainPlayingVideo(doc) {
  ...
  for (const { video } of videos) {
    if (!playing(video) || !video.matches('video.video-overlay')) continue;
    const overlay = video.closest('div.video-overlay'), player = overlay?.parentElement;
    if (!player || player === doc.body || player === doc.documentElement || !player.contains(main.video) || !overlay.querySelector('.video-overlay-skip')) continue;
    const rect = visibleRect(overlay, doc);
    if (!rect) continue;
    const overlap = Math.max(0, Math.min(rect.right, main.rect.right) - Math.max(rect.left, main.rect.left)) * Math.max(0, Math.min(rect.bottom, main.rect.bottom) - Math.max(rect.top, main.rect.top));
    if (overlap >= main.rect.width * main.rect.height * 0.8) return video;
  }
  return playing(main.video) ? main.video : null;
}
...
function matchingControl(node, doc, player) {
  ...
  // "Skip This Video" is an ad action only on this recognized, ready overlay.
  if (node.matches('.video-overlay-skip.skippable') && labels.some(label => /^skip (?:this )?video[.!»›→\s]*$/.test(label))) return true;
...
export function clickableSkip(node, doc, video = mainPlayingVideo(doc)) {
  ...
  const overlayControl = node.closest('.video-overlay-skip');
  if (overlayControl && (overlayControl !== node || !node.matches('.skippable') || !video.matches('video.video-overlay') || !node.closest('div.video-overlay') || node.closest('div.video-overlay') !== video.closest('div.video-overlay'))) return false;
