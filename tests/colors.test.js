...
test('color validation rejects unsafe or unsupported values; damaged saved colors fall back safely', () => {
  ...
  for (const color of [null, undefined, 0, {}, '#abc', 'red', '#12345678', ' #123456', '#123456; color:red', '#gggggg']) {
    ...
    if (color !== undefined) assert.throws(() => parseImport(JSON.stringify({ format: 'cumrocket.profiles', version: 1, scope: 'all', profiles: [p] }), 'all'));
  }
...
test('renderer replaces custom groups and styles, escapes invalid colors and cleans up on stop', () => {
  ...
  assert.equal(win.CSS.highlights.get('cumrocket-positive-123456').has(range), true);
  ...
  assert.equal(win.CSS.highlights.has('cumrocket-positive-123456'), false);
  assert.deepEqual(renderer.positiveRanges, []);
  assert.match(styles[0].textContent, /cumrocket-negative-abcdef/);
  renderer.paint([{ kind: 'positive', color: 'red; } body {', range }]);
  assert.equal(styles.length, 0);
  assert.equal(win.CSS.highlights.has('cumrocket-matches'), true);
  renderer.clear();
