...
test('overlay ad player selects its playing video above paused main content and waits for skip readiness', async t => {
  const browser = await chromium.launch({ channel: 'chromium', headless: true }); t.after(() => browser.close());
  const page = await browser.newPage({ viewport: { width: 1600, height: 900 } });
  const bundle = await build({ entryPoints: ['src/ads/skip.js'], bundle: true, write: false, format: 'iife', globalName: 'ads' });
  await page.setContent(`<div id="hlsplayer" style="position:relative;width:800px;height:450px">
    ...
    <div class="video-overlay" style="position:absolute;inset:0">
      ...
      <div id="anc-tst-skip_ad-btn" class="video-overlay-skip" style="position:absolute;bottom:20px;right:20px;background:white;padding:10px"><div class="video-overlay-skip-txt noselect">Skip ad</div></div>
    </div><button id="unrelated" style="position:absolute;left:20px;bottom:20px">Skip This Video</button></div>`);
  await page.addScriptTag({ content: bundle.outputFiles[0].text });
  await page.evaluate(async () => {
    const canvas = document.createElement('canvas'); canvas.width = 800; canvas.height = 450;
    setInterval(() => canvas.getContext('2d').fillRect(0, 0, 800, 450), 30);
    const video = document.querySelector('video.video-overlay'); video.srcObject = canvas.captureStream(30); await video.play();
    window.skips = 0; window.wrong = 0;
    document.querySelector('.video-overlay-skip').addEventListener('click', event => { event.stopPropagation(); skips++; });
    document.querySelector('#unrelated').addEventListener('click', () => wrong++);
    window.skipper = new ads.AdSkipper(document);
  });
  await page.waitForFunction(() => document.querySelector('video.video-overlay').currentTime > 0);
  assert.equal(await page.evaluate(() => ads.mainPlayingVideo(document) === document.querySelector('video.video-overlay')), true);
  await page.evaluate(() => { skipper.configure(true); skipper.scan(); });
  assert.equal(await page.evaluate(() => skips), 0);
  await page.evaluate(() => { document.querySelector('.video-overlay-skip').classList.add('skippable'); skipper.scan(); skipper.scan(); });
  assert.equal(await page.evaluate(() => skips), 1);
  await page.evaluate(() => { document.querySelector('.video-overlay-skip').classList.remove('skippable'); skipper.scan(); document.querySelector('.video-overlay-skip-txt').textContent = 'Skip This Video'; document.querySelector('.video-overlay-skip').classList.add('skippable'); skipper.scan(); });
  assert.equal(await page.evaluate(() => skips), 2);
  assert.equal(await page.evaluate(() => wrong), 0);
  await page.evaluate(() => { document.querySelector('video.video-overlay').pause(); document.querySelector('.video-overlay-skip').classList.remove('skippable'); skipper.scan(); document.querySelector('.video-overlay-skip').classList.add('skippable'); skipper.scan(); });
  assert.equal(await page.evaluate(() => skips), 2);
  await page.evaluate(async () => {
    const overlay = document.querySelector('div.video-overlay'), video = overlay.querySelector('video');
    overlay.style.cssText = 'position:absolute;left:850px;top:0;width:600px;height:400px';
    video.style.cssText = 'width:600px;height:400px';
    await video.play();
  });
  assert.equal(await page.evaluate(() => ads.mainPlayingVideo(document)), null);
  await page.evaluate(() => skipper.dispose());
});
