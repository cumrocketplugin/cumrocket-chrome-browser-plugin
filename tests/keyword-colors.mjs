...
export async function checkKeywordColors(context, page, id, url) {
  ...
  for (const surface of ['popup', 'sidepanel']) {
    ...
    await content.waitForFunction(() => CSS.highlights.get('cumrocket-positive-8fc9ff')?.size === 1 && CSS.highlights.get('cumrocket-positive-123456')?.size === 1);
    ...
    await page.getByRole('button', { name: 'Save profile', exact: true }).click();
    await page.getByText('Profile saved.', { exact: true }).waitFor();
    await content.waitForFunction(() => CSS.highlights.get('cumrocket-positive-abcdef')?.size === 1 && !CSS.highlights.has('cumrocket-positive-123456'));
