...
const profileDir = await mkdtemp(join(tmpdir(), 'cumrocket-test-'));
const errors = [];
let context;
...
try {
  ...
  await waitFor(site, () => CSS.highlights.get('cumrocket-matches')?.size === 4);
  assert.deepEqual(await site.evaluate(() => [...CSS.highlights.get('cumrocket-matches')].map(r => r.toString())), ['GPU', 'inference', 'data center', 'GPU']);
  assert.deepEqual(await site.evaluate(() => [...CSS.highlights.get('cumrocket-negative')].map(r => r.toString())), ['gaming']);
  ...
  await waitFor(site, () => CSS.highlights.get('cumrocket-matches')?.size === 6);
  ...
  await panel.getByRole('button', { name: 'Save profile' }).click();
  await waitFor(site, () => CSS.highlights.get('cumrocket-matches')?.size === 3);
  ...
  await panel.getByLabel('Highlight pages').check();
  await waitFor(site, () => CSS.highlights.get('cumrocket-negative')?.size === 2);
  ...
  await waitFor(site, () => !CSS.highlights.has('cumrocket-matches') && CSS.highlights.get('cumrocket-negative')?.size === 6);
  await panel.locator('#edit-profile').click();
  await panel.getByLabel('Profile enabled', { exact: true }).uncheck();
  await panel.getByRole('button', { name: 'Save profile' }).click();
  await panel.getByText('Profile saved.', { exact: true }).waitFor();
  await waitFor(site, () => CSS.highlights.get('cumrocket-negative')?.size === 4);
  await panel.locator('#edit-profile').click();
  await panel.getByLabel('Profile enabled', { exact: true }).check();
  await panel.getByRole('button', { name: 'Save profile' }).click();
  await panel.getByText('Profile saved.', { exact: true }).waitFor();
  await panel.evaluate(async id => { await chrome.runtime.sendMessage({ type: 'profile.delete', id }); }, secondary);
  await waitFor(site, () => CSS.highlights.get('cumrocket-matches')?.size === 4 && CSS.highlights.get('cumrocket-negative')?.size === 2);
  ...
  const isolated = worlds.find(world => world.origin === `chrome-extension://${id}` || world.name === 'CumRocket');
  ...
  await waitFor(importPage, () => CSS.highlights.get('cumrocket-matches')?.size === 2);
  ...
  await waitFor(importPage, () => [...CSS.highlights.get('cumrocket-matches')].map(r => r.toString()).join() === 'inference');
  assert.deepEqual(await importPage.evaluate(() => [...CSS.highlights.get('cumrocket-negative')].map(r => r.toString())), ['data center']);
  ...
  assert.deepEqual(await importPage.evaluate(() => [...CSS.highlights.get('cumrocket-matches')].map(r => r.toString())), ['inference']);
  ...
  assert.equal(await importPage.evaluate(() => CSS.highlights.has('cumrocket-matches') || CSS.highlights.has('cumrocket-negative')), false);
  ...
  await activeWorker.evaluate(async one => {
    const saved = (await chrome.storage.local.get('cumrocket.state'))['cumrocket.state'];
    await chrome.storage.local.set({ 'cumrocket.state': { ...saved, profiles: one.profiles } });
    const [tab] = await chrome.tabs.query({ url: 'http://127.0.0.1/*' });
  ...
  for (const term of ['inference', 'GPU']) {
    ...
    await current.evaluate(async ({ profile, term }) => {
      const saved = (await chrome.storage.local.get('cumrocket.state'))['cumrocket.state'];
      await chrome.storage.local.set({ 'cumrocket.state': { ...saved, profiles: [{ ...profile, positiveKeywords: [term], negativeKeywords: [] }] } });
    ...
    await importPage.waitForFunction(term => {
      const words = [...(CSS.highlights.get('cumrocket-matches') ?? [])].map(r => r.toString());
      return words.length === (term === 'GPU' ? 2 : 1) && words.every(word => word === term) && !CSS.highlights.has('cumrocket-negative');
    }, term);
  }
  ...
  const samples = [
    ...
    ['mention', null, '(@cumrocket) name@example.com', ['@cumrocket']],
  ...
  for (const [type, value, text, expected] of samples) {
    ...
    try {
      await importPage.waitForFunction(expected => JSON.stringify([...(CSS.highlights.get('cumrocket-matches') ?? [])].map(r => r.toString())) === JSON.stringify(expected), expected);
    } catch (error) {
      const actual = await importPage.evaluate(() => [...(CSS.highlights.get('cumrocket-matches') ?? [])].map(r => r.toString()));
  ...
  await criteriaPanel.evaluate(async () => {
    ...
    await chrome.storage.session.set({ [`cumrocket.scroll.v1.${tab.id}`]: { enabled: true, paused: true, speed: 600, pending: { target: 'https://example.invalid/next' } } });
  });
  await context.close();
  context = await launch();
  const freshWorker = context.serviceWorkers()[0] ?? await context.waitForEvent('serviceworker');
  const sessions = await freshWorker.evaluate(async () => Object.entries(await chrome.storage.session.get(null)).filter(([key]) => key.startsWith('cumrocket.scroll.v1.')).map(([, value]) => value));
  assert.ok(sessions.every(state => !state.enabled && !state.pending));
  const colorProfiles = await freshWorker.evaluate(async () => (await chrome.storage.local.get('cumrocket.state'))['cumrocket.state'].profiles.filter(p => p.name.startsWith('Colors ')));
  ...
  const visitsAfterRestart = await freshWorker.evaluate(async () => ({
    local: (await chrome.storage.local.get('cumrocket.visits.history.v1'))['cumrocket.visits.history.v1'],
    session: (await chrome.storage.session.get('cumrocket.visits.history.v1'))['cumrocket.visits.history.v1']
  }));
