...
test('serialized import persistence is atomic and preserves settings on failure', async () => {
  ...
  const store = createStore({ async get() { return { 'cumrocket.state': structuredClone(saved) }; }, async set(value) { if (fail) throw Error('disk'); saved = structuredClone(value['cumrocket.state']); } });
  ...
  assert.deepEqual((await createStore({ async get() { return { 'cumrocket.state': saved }; } }).read()).profiles, [first, second]);
