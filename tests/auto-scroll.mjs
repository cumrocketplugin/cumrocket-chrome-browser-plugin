...
export async function checkAutoScroll(context, panel, origin) {
  ...
  assert.equal((await panel.evaluate(async id => (await chrome.storage.session.get(`cumrocket.scroll.v1.${id}`))[`cumrocket.scroll.v1.${id}`], tabId)).paused, true);
  await site.close();
  await panel.waitForFunction(async id => !(await chrome.storage.session.get(`cumrocket.scroll.v1.${id}`))[`cumrocket.scroll.v1.${id}`], tabId);
  await other.close();
}
