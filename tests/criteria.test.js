...
const cases = [
  ...
  ['mention', {}, '(@CumRocket), @犬 @café name@example.com', ['@CumRocket', '@犬', '@café'], '@ x@dog @@dog'],
...
test('validation rejects malformed criteria before save/import; evaluation fails closed', () => {
  ...
  for (const c of invalid) {
    ...
    p.rules.criteria = [c];
    assert.throws(() => parseImport(JSON.stringify({ format: 'cumrocket.profiles', version: 1, scope: 'single', profiles: [p] }), 'single'));
  }
