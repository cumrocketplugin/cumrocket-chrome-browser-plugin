import { entertainerPreferences } from '../src/entertainers/directory.js';
...
test('storage rejects unknown versions without overwriting saved data', async () => {
  ...
  await area.set({ 'cumrocket.state': { schemaVersion: 99, enabled: false } });
  const store = createStore(area);
  await assert.rejects(store.update(s => ({ ...s, enabled: true })), /Unsupported storage version/);
  assert.deepEqual(await area.get('cumrocket.state'), { 'cumrocket.state': { schemaVersion: 99, enabled: false } });
});
...
test('legacy preferences gain popup default without changing profiles, model or key', async () => {
  ...
  await area.set({ 'cumrocket.state': saved, 'cumrocket.apiKey': 'test-placeholder' });
  const store = createStore(area);
  assert.deepEqual(await store.read(), { ...saved, entertainers: entertainerPreferences(), preferences: { model: 'custom-model', sidebar: false, tracking: false, eyeballLevel: 50, autoPauseColors: [...DEFAULT_AUTO_PAUSE_COLORS], provider: 'openai', providerModels: { openai: 'custom-model' } } });
