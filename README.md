...
Open **Captions** in the popup or sidebar and enable **Analyze captions on this website**. The switch is saved per hostname and works independently of highlighting. Collection starts on enabled pages and includes visible caption elements, video card titles, and subtitle/caption tracks exposed by the player. Dynamically loaded captions are collected while you browse. It does not fetch unloaded pages, transcribe audio, read text in images, or access captions inside inaccessible frames/players.

Choose **Generate report** for keyword frequencies, keywords and phrases used once, repeated captions, and repeated fragments of 2–6 words across otherwise different captions. Counts ignore case and punctuation; phrase boundaries omit common English filler words. Numbered examples link findings to the collected caption list. Repeated scans of an unchanged element do not add counts; separate elements or changed text do. Each list shows up to 50 results.

Collected text stays in the tab's memory and resets on navigation/reload, disabling collection, or **Start over** (which immediately collects the current page again). Collection is limited to 1,000 captions of 500 characters each; the report indicates when content is limited. **Download report** saves a JSON report, including its caption evidence.

When the selected provider has a saved API key, enable **Advanced AI analysis**, then click **Send captions and analyze**. This sends the first 100 collected captions to your selected OpenAI or Anthropic model for themes, paraphrases, repeated ideas, and distinctive wording with caption IDs. API charges may apply. No request runs merely because a key exists or the checkbox is enabled. AI interpretations can be checked against the local caption evidence.
